...
class TransferStep:
    def __init__(self, source_id: str, source_well: str, destination_id: str, destination_well: str, volume: int):
        """
...
class Transfer:
    ...
    def simulate(self, source_plates: Dict[str, Plate], destination_well_number: int, save_plate=False, save_dir=''):
        destination_plates = {}
        for transfer_step in self.transfer_steps:
            ...
            try:
                source_plate.consume_well(transfer_step.source_well, transfer_step.volume)
            except ValueError:
                raise ValueError(
            ...
            if transfer_step.destination_id not in destination_plates.keys():
                if destination_well_number == 96:
                    destination_plates[transfer_step.destination_id] = Plate96(max_vol=9999999999, dead_vol=0)
                elif destination_well_number == 384:
                    destination_plates[transfer_step.destination_id] = Plate384(max_vol=9999999999, dead_vol=0)
                else:
                    raise NotImplementedError('Can only use 96 or 384 well plates')

            # add to destination plate
            try:
                destination_plates[transfer_step.destination_id].fill_well(transfer_step.destination_well, source_cmps,
                                                                           transfer_step.volume)
            except Exception:
                ValueError(f'Invalid transfer {transfer_step} for given destination plate')
            ...
            if save_plate is True:
                if save_dir == '':
                    ...
                    for name, plate in destination_plates.items():
                        plate.to_csv(os.path.join(save_dir, f'plate_layout_{name}.csv'), save_volumes=True)
    ...
    def check_correct(self, source_plates: Dict[str, Plate], destination_plates: Dict[str, Plate]):
        pass
...
if __name__ == '__main__':
    ...
    def import_echo_transfer_file(file):
        with open(file, 'r') as file:
            ...
            for line in reader:
                transfer_steps.append(TransferStep(line[0], line[1], line[2], line[3], int(line[4])))
