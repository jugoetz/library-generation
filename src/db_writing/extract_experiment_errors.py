...
def read_mobias_analysis_errors(path: Path, plate_number: int) -> list:
    ...
    results.loc[
        ~results[f"{internal_standard_number} Area"].between(
            mean_response_area * 0.5, mean_response_area * 1.5
        ),
        ["error_3"],
    ...
    results.loc[
        ~results[f"{internal_standard_number} Area"].between(
            mean_response_area * 0.2, mean_response_area * 1.8
        ),
        ["error_4"],
    ] = "ERROR: IS response differs >80% from mean"
...
def get_mobias_errors(exp_nr: int) -> list:
    ...
    for plate in plate_list:
        error_list += read_mobias_analysis_errors(
            PLATES_DIR / plate[1] / plate[2], int(plate[0])
        )
    return error_list
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_dir", type=Path, help="Path to the experiment directory", default=exp_dir
    )
    parser.add_argument(
        "--skip_nexus",
        action="store_true",
